  ...
  return (
    <Card className="mx-auto w-full max-w-md overflow-hidden border-primary/15 shadow-lg shadow-primary/5">
      ...
      <CardContent className="pt-6"><div key={step} className="animate-in fade-in-0 slide-in-from-right-4 duration-300 motion-reduce:animate-none">
        {step === "email" ? <form onSubmit={sendOtp}><FieldGroup><Field><FieldLabel htmlFor="reset-email">{t("email")}</FieldLabel><Input id="reset-email" type="email" autoComplete="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder={t("emailPlaceholder")} className="h-11" autoFocus required /></Field><Button type="submit" className="h-11 w-full" disabled={requestOtp.isPending}>{requestOtp.isPending ? `${t("sendOtp")}…` : t("sendOtp")}</Button></FieldGroup></form>
        : step === "otp" ? <form onSubmit={verify}><FieldGroup><div className="rounded-lg bg-muted/50 px-3 py-2 text-sm text-muted-foreground">{email}</div><Field><FieldLabel htmlFor="reset-otp">{t("otp")}</FieldLabel><Input id="reset-otp" inputMode="numeric" autoComplete="one-time-code" value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, ""))} placeholder={t("otpPlaceholder")} className="h-12 text-center text-lg tracking-[0.35em]" autoFocus required /></Field><Button type="submit" className="h-11 w-full" disabled={verifyOtp.isPending}>{verifyOtp.isPending ? `${t("verify")}…` : t("verify")}</Button><div className="flex items-center justify-between"><Button type="button" variant="ghost" size="sm" onClick={() => setStep("email")}><ArrowLeft className="size-4" />{t("email")}</Button><Button type="button" variant="ghost" size="sm" disabled={requestOtp.isPending} onClick={() => void sendOtp()}>{t("sendOtp")}</Button></div></FieldGroup></form>
        : <form onSubmit={reset}><FieldGroup><div className="flex items-center gap-2 rounded-lg bg-emerald-500/10 px-3 py-2 text-sm text-emerald-700 dark:text-emerald-300"><Check className="size-4" />{toast("otpVerifiedTitle")}</div><Field><FieldLabel htmlFor="new-password">{t("newPassword")}</FieldLabel><Input id="new-password" type="password" autoComplete="new-password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="h-11" autoFocus required /></Field><Field><FieldLabel htmlFor="confirm-password">{t("confirmPassword")}</FieldLabel><Input id="confirm-password" type="password" autoComplete="new-password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="h-11" required /></Field><Button type="submit" className="h-11 w-full" disabled={resetPassword.isPending}>{resetPassword.isPending ? t("resetting") : t("reset")}</Button><Button type="button" variant="ghost" size="sm" className="justify-self-start" onClick={() => { setNewPassword(""); setConfirmPassword(""); setStep("otp"); }}><ArrowLeft className="size-4" />{t("otp")}</Button></FieldGroup></form>}
