  ...
  return (
    <div className="mt-2 space-y-1.5 rounded-md border border-primary/10 bg-primary/5 p-1.5">
      <p className="text-[10px] leading-tight text-muted-foreground">
        Select 2+ questions, then create one follow-up that depends on all or any of them.
      </p>
      <div className="grid grid-cols-[118px_minmax(0,1fr)] items-center gap-1.5">
        <Select
          value={props.logicType}
          onValueChange={(value) => props.onLogicTypeChange(value as ShowCondition["logicType"])}
          disabled={props.disabled}
        >
          <SelectTrigger className="h-7 w-[118px] min-w-[118px] px-2 text-[10px] [&_svg]:ml-auto [&_svg]:size-3">
      ...
      <p className="text-[10px] text-muted-foreground">
        {props.logicType === "AND"
          ? "All selected parent answers must match."
          : "One matching parent answer is enough."}{" "}
